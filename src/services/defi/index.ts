...
// USDT/XAUT-focused token watchlist
const STABLECOIN_IDS = new Set(["tether", "usd-coin", "dai", "frax"]);
const GOLD_TOKEN_IDS = new Set(["pax-gold", "tether-gold"]);
const FOCUS_TOKEN_IDS = new Set([...STABLECOIN_IDS, ...GOLD_TOKEN_IDS]);
...
const { app, log, start } = createService({
  ...
  routes: {
    "POST /scan": {
      price: "$0.015",
      description: "Scan DeFi markets for USDT/XAUT yield and stablecoin opportunities",
    },
  },
  healthExtra: () => ({
    cacheAge: {
...
function scoreToken(token: CoinGeckoToken): ScoredToken {
  ...
  // Boost for USDT/XAUT-focused tokens (stablecoin yield opportunities)
  if (FOCUS_TOKEN_IDS.has(token.id)) {
    score += 3;
  } else if (STABLECOIN_IDS.has(token.id)) {
    score += 2;
  }
  ...
  if (FOCUS_TOKEN_IDS.has(token.id) && score >= 4) {
    suggestedAction = "YIELD OPPORTUNITY — stablecoin/gold token with strong signals";
  } else if (score >= 7 && change24h > 5) {
    suggestedAction = "MOMENTUM ENTRY — strong multi-timeframe trend";
  } else if (score >= 4) {
    suggestedAction = "WATCH CLOSELY — building momentum";
  } else if (change24h < -5) {
    suggestedAction = "POTENTIAL REVERSAL — dip opportunity?";
  } else {
    suggestedAction = "HOLD / MONITOR";
  }
