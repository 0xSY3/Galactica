...
import { initializeAllWallets, isDemoMode } from "../lib/wdk-wallet.js";
...
import { startRepaymentCollector, stopRepaymentCollector } from "./contract-executor.js";
import { initializeProtocols } from "../lib/wdk-protocols.js";
...
// ─── CORS (coordinator is free — WDK payments happen coordinator→sub-services) ─

conditionalPaywall(app, undefined, {});

// ─── Initialize WDK wallets and protocol modules for all agents ─
initializeAllWallets().catch(err => log.warn("WDK init error (non-fatal)", { error: (err as Error).message }));
initializeProtocols();
...
const server = app.listen(port, "0.0.0.0", () => {
  const ip = getLocalIP();
  log.info("Galactica Strategy Engine started", {
    port,
    localUrl: `http://localhost:${port}`,
    networkUrl: ip ? `http://${ip}:${port}` : undefined,
    wdkMode: isDemoMode() ? "demo" : "live",
    tracks: ["agent-wallets", "lending", "defi", "tipping"],
  });
  initTelegram();
  initMoltbook();
});
