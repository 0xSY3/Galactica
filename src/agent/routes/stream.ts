...
export function registerStreamRoutes(app: Application): void {
  app.get("/stream", async (req, res) => {
    ...
    try {
      const fromAddr = getAgentAddress("hunter");

      send("start", { topic, huntId, timestamp: new Date().toISOString(), services: 5, fromAddr });
      ...
      send("paying", { service: "news-agent", amount: newsP.effectivePrice, baseAmount: newsP.basePrice, multiplier: newsP.multiplier, port: 4004, fromAddr });
      send("paying", { service: "crypto-sentiment", amount: sentP.effectivePrice, baseAmount: sentP.basePrice, multiplier: sentP.multiplier, port: 4001, fromAddr });
      send("paying", { service: "crypto-sentiment-v2", amount: sent2P.effectivePrice, baseAmount: sent2P.basePrice, multiplier: sent2P.multiplier, port: 4006, fromAddr });
      send("paying", { service: "polymarket-alpha-scanner", amount: polyP.effectivePrice, baseAmount: polyP.basePrice, multiplier: polyP.multiplier, port: 4002, fromAddr });
      send("paying", { service: "defi-alpha-scanner", amount: defiP.effectivePrice, baseAmount: defiP.basePrice, multiplier: defiP.multiplier, port: 4003, fromAddr });
      send("paying", { service: "whale-agent", amount: whaleP.effectivePrice, baseAmount: whaleP.basePrice, multiplier: whaleP.multiplier, port: 4005, fromAddr });
      ...
      for (const ext of extAgents) {
        ...
        send("paying", { service: ext.displayName, amount: extDp.effectivePrice, baseAmount: extDp.basePrice, multiplier: extDp.multiplier, port: null, url: ext.url, fromAddr, external: true });
      }
      ...
      const builtinSettled = Promise.allSettled([
        ...
        callWhale(getAgentAddress("hunter"), signal),
      ]);
      ...
      for (const entry of resultEntries) {
        const toAddr = getAgentAddress(PORT_TO_AGENT[entry.port] ?? "unknown");
      ...
      for (let i = 0; i < extAgents.length; i++) {
        ...
        send("result", { service: agent.displayName, data: extRes?.data ?? null, txHash: extRes?.txHash, paid: extRes?.paid ?? false, fromAddr, external: true });
        if (fromAddr) {
          recordTx({
            timestamp: new Date().toISOString(),
