...
import { approveLoan, depositToLendingPool, tipCreator, depositToVault, rebalanceVault, depositToTippingPool } from "./contract-executor.js";
import { swapTokens, bridgeUSDT, aaveDeposit } from "../lib/wdk-protocols.js";
...
async function runHunt(): Promise<void> {
  ...
  try {
    ...
    const paymentLog: PaymentLog = {
      totalPaid: walletClient ? `${dp.reduce((s, p) => s + parseFloat(p.effectivePrice.replace("$", "")), 0).toFixed(4)} USDT` : "demo mode",
    ...
    try {
      ...
      if (confidence >= 75 && topicLower.includes("lending")) {
        const result = await depositToLendingPool(10);
        log.info("autopilot contract action: lending deposit", { result: result.action, demoMode: result.demoMode });
        ...
        if (creditScore >= 50 || creditSignal === "ACCUMULATION") {
          const approveResult = await approveLoan(-1);
          log.info("autopilot contract action: loan approval", { result: approveResult.action, demoMode: approveResult.demoMode });
        } else {
          log.info("autopilot: loan approval skipped — credit score too low", { creditScore });
        }
      }

      // Tipping: use engagement monitor signal when available, fall back to news article count
      const engagementLevel = alpha.breakdown.sentiment?.engagementLevel ?? "low";
      const tipRec = alpha.breakdown.sentiment?.tipRecommendation;
      const shouldTip = engagementLevel === "viral" || engagementLevel === "high"
        || (confidence >= 55 && (alpha.breakdown.news?.articleCount ?? 0) >= 3);

      if (shouldTip) {
        const creatorAddr = config.tipping.creatorAddress || getAgentAddress("hunter");
        const tipAmount = tipRec ? parseFloat(tipRec.replace(/[^0-9.]/g, "")) || config.tipping.defaultTipUsdt : config.tipping.defaultTipUsdt;
        const reason = tipRec ? `autopilot: ${tipRec}` : `autopilot tip: ${topic}`;
        const result = await tipCreator(creatorAddr, tipAmount, reason);
        log.info("autopilot contract action: creator tip", { result: result.action, demoMode: result.demoMode });
      }

      if (whaleSignal === "ACCUMULATION" || topicLower.includes("yield")) {
        const result = await depositToVault(5);
        log.info("autopilot contract action: vault deposit", { result: result.action, demoMode: result.demoMode });

        // Aave deposit for higher yield when signal is strong
        if (confidence >= 70) {
          const aaveResult = await aaveDeposit(5);
          log.info("autopilot contract action: Aave deposit", { result: aaveResult.action, demoMode: aaveResult.demoMode });
        }
      }

      // Fund tipping pool before tipping
      if (shouldTip) {
        const poolResult = await depositToTippingPool(config.tipping.defaultTipUsdt * 2);
        log.info("autopilot contract action: tipping pool deposit", { result: poolResult.action, demoMode: poolResult.demoMode });
      }

      // WDK Protocol: periodic rebalancing via Velora swap
      if (topicLower.includes("rebalance") || topicLower.includes("swap")) {
        const result = await swapTokens(config.wdk.usdtContract, config.wdk.xautContract, 10);
        log.info("autopilot contract action: token swap", { result: result.action, demoMode: result.demoMode });
      }

      // WDK Protocol: cross-chain bridge when multi-chain topic detected
      if (topicLower.includes("bridge") || topicLower.includes("cross-chain")) {
        const result = await bridgeUSDT(5, config.bridgeDefaultChain);
        log.info("autopilot contract action: USDT bridge", { result: result.action, demoMode: result.demoMode });
      }
    } catch (contractErr) {
      log.warn("autopilot contract execution failed (non-critical)", { error: (contractErr as Error).message });
    }
