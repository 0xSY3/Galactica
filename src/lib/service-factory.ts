...
export interface ServiceOpts {
  ...
  /** Port number */
  port: number;
  /** WDK paywall routes (e.g. { "POST /analyze": { price, description } }) */
...
export function createService(opts: ServiceOpts): ServiceInstance {
  ...
  // Apply network to all route defs
  const routes: Record<string, RouteConfig> = {};
  for (const [key, val] of Object.entries(opts.routes)) {
    routes[key] = { ...val, network: val.network ?? config.wdk.network };
  }

  conditionalPaywall(app, undefined, routes);
