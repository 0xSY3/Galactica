...
export interface WDKPaymentBody {
  accepts: Array<{
...
/** @deprecated Use WDKFetchResult */
export type X402Body = WDKPaymentBody;

export interface WDKFetchResult {
  ok: boolean;
  status: number;
  data: unknown;
  paid: boolean;
...
/** @deprecated Use WDKFetchResult */
export type X402FetchResult = WDKFetchResult;
...
export interface AlphaSynthesis {
  ...
  breakdown: {
    sentiment: Pick<SentimentResult, "label" | "score" | "confidence" | "engagementLevel" | "tipRecommendation"> | null;
    ...
    whale: { signal: string; whaleCount: number; totalVolume: string; creditScore: number; riskLevel: string } | null;
