...
function AccuracyChart({ buckets }: { buckets: AccuracyBucket[] }) {
  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: `repeat(${buckets.length}, 1fr)`, gap: ".75rem" }}>
      {buckets.map((b) => (
        <div key={b.range} style={{ textAlign: "center" }}>
          ...
          <div style={{ fontFamily: "var(--mono)", fontSize: ".72rem", color: "var(--text2)" }}>
            {b.range}%
          </div>
          <div style={{ fontFamily: "var(--mono)", fontSize: ".65rem", color: "var(--text3)", marginTop: ".1rem" }}>
            {b.count}
          </div>
        </div>
      ))}
      ...
      <div className="memory-panel">
        ...
        >
          {[
            ...
            { label: "Verified", value: stats?.verifiedEntries ?? 0 },
            { label: "Patterns", value: stats?.patterns ?? 0 },
            { label: "Active", value: stats?.activePatterns ?? 0 },
          ].map((s) => (
            <div
              key={s.label}
              style={{
            ...
            >
              <div style={{ fontSize: "1.2rem", fontWeight: 700, color: "var(--accent2)", fontFamily: "var(--mono)" }}>
                {statsLoading ? "--" : s.value}
              </div>
              <div style={{ fontSize: ".65rem", color: "var(--text3)", textTransform: "uppercase", letterSpacing: ".05em", marginTop: ".25rem" }}>
                {s.label}
              </div>
            </div>
          ))}
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1.25rem" }}>
          <div>
            <h4 style={{ fontSize: ".75rem", color: "var(--text2)", marginBottom: ".6rem", fontWeight: 600 }}>
              Top Patterns
            </h4>
